...
class TestFrivol < Test::Unit::TestCase
  def setup
    ...
    Frivol::Config.redis_config = { :thread_safe => true, :db => 10 } # This will connect to a default Redis setup, otherwise set to { :host => "localhost", :port => 6379 }, for example
  ...
  should "have thread safe counters" do
    ...
    if defined? FAKE_REDIS
      skip "Skipped because running tests with a fake Redis"
    else
